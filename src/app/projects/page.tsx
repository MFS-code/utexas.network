...
const SUBMIT_ISSUE_URL = (() => {
  const title = encodeURIComponent('Project submission: [project name]');
  const body = encodeURIComponent(
    `## Project / Org Submission\n\n` +
    `**Project name:**\n\n` +
    `**Short description:**\n\n` +
    `**Website URL:**\n\n` +
    `**Logo / image URL:**\n\n` +
    `**Member IDs** (comma-separated, at least one must be in the network):\n\n` +
    ...
    `- GitHub:\n\n` +
    `---\n` +
    `_Submitted via utexas.network/projects_`
  );
  return `https://github.com/MFS-code/utexas.network/issues/new?title=${title}&body=${body}&labels=project-submission`;
})();
